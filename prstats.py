...
parser.add_argument("-o", "--owner", help="Owner of repository",
                    action='store_true', default='alenaPy')
parser.add_argument("-r", "--repo", help="Name of repository",
                    action='store_true', default='devops_lab')
...
if args.pull or args.all:
    ...
    for i in range(lenstr):
        ...
        print(icreatedat + " - " + ititle)

        if args.pullclose or args.all:
            iclosedate = r.json()[i]['closed_at']
            print("Pull closed: " + str(iclosedate))

        if args.weekday or args.all:
            iweekday = datetime.datetime.strptime(icreatedat, "%Y-%m-%dT%H:%M:%SZ")
            print("Created day of week:", calendar.day_name[datetime.datetime.weekday(iweekday)])
